...
int main(int argc, char *argv[]){
	...
	if(argc<8){
		fprintf(stderr,"Please enter all args eg: ./multi file1.txt rows1 cols1 file2.txt rows2 cols2 ans.txt");
		exit(1);
		}
	...
	//check dims
	if (width1!=height2){
		fprintf(stderr,"Please recheck the dimensions.. cols_matA must be equal to rows_matB\n");
		exit(EXIT_FAILURE);
		}
